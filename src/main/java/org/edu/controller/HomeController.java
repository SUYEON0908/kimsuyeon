...
public class HomeController {
	...
	public String weare() {
		//외부에서 /weare경로로 접근했을때.
		return "sample/weare";//리턴 반환값이 상대경로의 파일명(prefix).jsp생략
	}
	@RequestMapping(value="/",method=RequestMethod.GET)
	public String index( ) {//메서드(함수)구성:리턴(반환출력)형태지정 매서드명(매개변수) {구현내용} string index//
		//상단 리퀘스트매핑에서 지정한 경로로 접근을 하게되면, 아래에 있는 index()매서드를 실행해라.
		//매서드명 제일 앞에 있는 public는 접근권한에 대한 키워드인데 public(공공-다른클래스에서 접근가능)
		//,private(내부-현재 HomeController클래스에서만 접근가능)
		//public string index() {}에서도 기본형태
